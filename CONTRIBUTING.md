...
Adding a system package to the Docker build is rare. It is only justified when
the build genuinely needs a new apt package that none of the pinned python or
git inputs pull in.

- Add the new package to `locks/apt-packages.txt` as a **bare, unversioned
  name**, for example `libdw-dev`. Do not add a version.
- Do not hand-edit, re-version, reorder, or remove any existing entry in
  `locks/apt-packages.txt` or any other lockfile under `locks/`. Those entries
  are generated by `scripts/bump.sh` on the trusted DGX Spark, which resolves
  exact versions through `apt-cache madison` against the pinned apt snapshot.
- The reviewed form of the change is the unversioned seed. `run-bump.yaml`
  resolves the exact version and writes the versioned entry back onto the
  branch before merge.
- If the package is a non-obvious, special build requirement (for example a
  dev header like `elfutils/libdwfl.h`), keep or add the package-specific
  contract test so the requirement cannot silently regress. Do not add
  one-off string tests for packages whose presence is already derivable from
  the lockfile.
