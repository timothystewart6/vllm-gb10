...
This document explains how contributions reach the GB10 runners without
...
The GB10 bump workflow treats contributor changes as untrusted. It imports only
...
All validators, `scripts/bump.sh`, and lockfile policy tests executed on GB10
...
flowchart TD
    ...
    K --> L[Run trusted bump generator on GB10]
...
| Workflow and generator scripts | Exact trusted `main` SHA | GB10 runner |
| Generated lockfiles | Untrusted output until reviewed | Integration PR |
| Write token | Trusted workflow steps only | GB10 runner after no contributor code has executed |
| Merged `main` code | Trusted by repository review policy | GB10 build and smoke-test jobs |
...
The push to `main` starts the GB10 image build and smoke test.
...
There is no safe pre-merge GB10 execution path for arbitrary contributor code
